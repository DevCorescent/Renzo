...
export function ScheduleTab({ workerId }: { workerId: string }) {
  ...
  if (phase.kind === "loading") {
    return <div className="flex justify-center py-16"><Loader2 className="size-5 animate-spin text-gray-400" /></div>;
  }
  if (phase.kind === "error") {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-red-100 bg-red-50/50 px-6 py-12 text-center">
        ...
        <button
          ...
          className="mt-3 inline-flex h-9 items-center gap-1.5 rounded border border-gray-300 bg-white px-3 text-sm text-gray-700 transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900/10"
        >
  ...
  if (!shift) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-white px-6 py-14 text-center">
        <span className="flex size-11 items-center justify-center rounded-full bg-gray-50 text-gray-300 ring-1 ring-gray-200"><CalendarClock className="size-5" /></span>
        <p className="mt-3 text-sm font-medium text-gray-700">No shift assigned</p>
      </div>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><CalendarClock className="size-4 text-gray-400" />{shift.name}</CardTitle>
      ...
      <CardBody className="space-y-4">
        ...
        <div>
          <p className="mb-2 text-[11px] font-medium uppercase tracking-wide text-gray-400">Working days</p>
          <div className="flex flex-wrap gap-1.5">
            {DAYS.map((d, i) => (
              <span
                key={d}
                className={`inline-flex size-8 items-center justify-center rounded-full text-[11px] font-medium ${
                  shift.workingDays.includes(i) ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-400"
                }`}
              >
                {d[0]}
              </span>
