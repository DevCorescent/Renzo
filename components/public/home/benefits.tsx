...
import type { BenefitsData } from "@/lib/cms/schema";

// `data` is required: the renderer always supplies it, and the fallback content
// lives in exactly one place (lib/cms/defaults.ts). A second copy here would be a
// duplicate that silently drifts from the published defaults.
export function Benefits({ data }: { data: BenefitsData }) {
  const items = data.items.filter((item) => !item.hidden);

  return (
    <section className="relative overflow-hidden bg-[#111315] py-10 sm:py-14 lg:py-16">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <MotionReveal className="mx-auto max-w-6xl text-center">
          ...
          <h2 className="mt-4 font-heading text-[clamp(2rem,6vw,3.75rem)] font-semibold leading-none tracking-tight text-white whitespace-nowrap">
            {data.titleLead}{" "}
            <span className="text-[#E8E5DE]">{data.titleAccent}</span>
          </h2>

          <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-[#B8BEC8]">
            {data.paragraph}
          </p>
        </MotionReveal>

        <MotionStagger className="mt-8 sm:mt-10 lg:mt-12 grid gap-6 md:gap-8 md:grid-cols-2 xl:grid-cols-3">
          {items.map((benefit) => {
            const Icon = cmsIcon(benefit.icon);
            ...
            return (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="mt-6 text-xl font-semibold lg:mt-8 lg:text-2xl lg:font-bold text-white">
                    {benefit.title}
                  </h3>
