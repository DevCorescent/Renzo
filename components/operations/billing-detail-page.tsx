...
import { denyIfOutOfScope, requireBranchScope } from "@/lib/branch-scope";
...
export async function BillingDetailPage({
  ...
  if (!operationsCapabilitiesFor(authUser.userType).canBill) redirect("/unauthorized");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900 dark:text-(--sa-text)">
        ...
        <CardBody>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <p className="font-medium text-gray-900 dark:text-(--sa-text)">
            ...
            {invoice.appointment ? (
              <div>
                ...
                <p className="text-gray-700 dark:text-(--sa-text)">
              ...
              <div>
                ...
                <p className="text-gray-700 dark:text-(--sa-text)">Counter sale</p>
            ...
            <tbody>
              {invoice.items.map((item) => (
                <tr key={item.id} className="border-b border-gray-50 last:border-0">
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-(--sa-text)">{item.name}</td>
          ...
          <div className="divide-y divide-gray-50">
            {invoice.payments.map((p) => (
              <div key={p.id} className="flex items-center justify-between px-4 py-3">
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-(--sa-text)">{METHOD_LABEL[p.method] ?? p.method}</p>
          ...
          <div className="divide-y divide-gray-50">
            {invoice.refunds.map((r) => (
              <div key={r.id} className="flex items-center justify-between px-4 py-3">
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-(--sa-text)">{METHOD_LABEL[r.method] ?? r.method}</p>
