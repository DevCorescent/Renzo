...
import { setSuperLeaveStatusAction, grantSuperLeaveAction } from "@/app/super-admin/leaves/actions";
import { LeaveRequestCard } from "./leave-request-card";
import { ReviewModal } from "./review-modal";
import { GrantLeaveModal } from "@/components/branch-leaves/grant-leave-modal";
import type { BranchLeave, LeaveStatus, LeaveTypeOption } from "@/components/branch-leaves/types";
...
export function SuperLeavesView({
  ...
  async function runStatus(id: string, status: "APPROVED" | "REJECTED" | "PENDING"): Promise<string | null> {
      ...
      <div className="flex justify-end">
        {leaveTypes.length > 0 && (
          <GrantLeaveModal
            ...
            onGrant={async (input) => {
              const res = await grantSuperLeaveAction(input);
              if (res.status === "success") {
                setToast(res.message);
                return null;
              }
      ...
      <div className="space-y-4">
        {groups.length === 0 ? (
          <p className="rounded-lg border border-dashed border-gray-200 bg-white px-4 py-8 text-center text-sm text-gray-500">
            No leave requests match these filters. You can still grant leave above.
          </p>
        ) : (
          groups.map((g) => {
