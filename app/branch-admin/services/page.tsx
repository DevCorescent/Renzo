        ...
        <div className="border-t border-gray-100 pt-3">
          ...
          {isActive && (
            <button
              ...
              className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg border border-gray-200 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50"
            >
              <Users className="size-3" /> Allocate Workers
            </button>
          )}
        </div>
      </div>
...
function AllocateWorkersModal({
  ...
  React.useEffect(() => {
    (async () => {
      try {
        // The branch comes from the signed-in admin's own session.
        const meRes = await fetch(API.auth.me);
        const meJson = await meRes.json();
        if (!meRes.ok) throw new Error(meJson?.message ?? "Failed to load session");

        const branchId: string | null = meJson.data?.user?.branchId ?? null;
        if (!branchId) throw new Error("Your account is not assigned to a branch");

        const res = await fetch(
          `${API.admin.workers}?branchId=${encodeURIComponent(branchId)}&limit=100`
        );
        const json = await res.json();
        if (!res.ok) throw new Error(json?.message ?? "Failed to load workers");

        const items: Worker[] = json.data?.items ?? [];
        const active = items.filter((w) => w.isActive);
        const current = new Set(
          active
            .filter((w) => w.services.some((s) => s.serviceId === serviceId))
            .map((w) => w.id)
        );
        ...
        setErr(e instanceof Error ? e.message : "Failed to load workers");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const changed = workers.filter((w) => assigned.has(w.id) !== initial.has(w.id));
  ...
  async function save() {
    ...
    setErr(null);
    try {
      // PUT replaces a worker's whole service list, so send their existing
      // services with this one added or removed.
      const failed: string[] = [];

      await Promise.all(
        changed.map(async (w) => {
          const others = w.services
            .map((s) => s.serviceId)
            .filter((id) => id !== serviceId);
          const serviceIds = assigned.has(w.id) ? [...others, serviceId] : others;

          const res = await fetch(API.admin.workerServices(w.id), {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
          ...
          if (!res.ok) {
            ...
            failed.push(
              `${w.firstName} ${w.lastName}: ${json?.message ?? "save failed"}`
            );
          }
        })
      );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <div>
            <h2 className="font-semibold text-gray-900">Allocate Workers</h2>
            <p className="mt-0.5 text-xs text-gray-500">
              Who performs <span className="font-medium text-gray-700">{serviceName}</span> at this branch?
            </p>
          </div>
          ...
          {loading ? (
            ...
            <>
              {workers.length > 6 && (
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-gray-200 py-2 pl-9 pr-3 text-sm focus:border-indigo-400 focus:outline-none"
                  />
                </div>
              )}
              ...
              <div className="max-h-72 space-y-1 overflow-y-auto">
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block truncate text-[11px] text-gray-400">
                          {w.employeeCode}
                          {w.designation ? ` · ${w.designation.name}` : ""}
                        </span>
                      </span>
                    </button>
                  );
                ...
                {visible.length === 0 && (
                  <p className="py-8 text-center text-sm text-gray-400">
                    {workers.length === 0
                      ? "No active workers at this branch yet."
              ...
              <p className="text-xs text-gray-400">
                {assigned.size} of {workers.length} selected
    ...
    <div className="space-y-6">
      ...
      {allocateSvc && (
        <AllocateWorkersModal
          serviceId={allocateSvc.id}
          serviceName={allocateSvc.name}
          onClose={() => setAllocateSvc(null)}
        />
      )}
    </div>
  );
